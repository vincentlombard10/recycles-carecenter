...
class SyncZendeskTicketsWithoutCommentsController extends Controller
{
    public function __invoke()
    {
        ...
        $tickets = Ticket::doesntHave('comments')->orderByDesc('id')->limit(20)->get();

        foreach($tickets as $t) {
            $ticketsData = $client->tickets()->find($t->id);
            $ticket = $ticketsData->ticket;
            $ticketMetric = $client->tickets($t->id)->metrics()->findAll()->ticket_metric;
            ...
            try {
                ...
                $t->comment_count = count($ticketComments);
                $t->save();
