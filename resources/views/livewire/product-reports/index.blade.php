    ...
    @if(count($reports))
        @foreach($reports as $report)
            <div class="Card_Support Card_Support--{{ $report->status }}">
                <div class="Card_Support--Body">
                    <div class="Card_Support--Main mb-2">
                        ...
                        <div>
                            <div><small>Créé le {{ date('d/m/Y à H:i', strtotime($report->created_at)) }}&nbsp;|&nbsp;<span class="fw-semibold">{{ $report->return->author?->username }}</span></small></div>
                            @if($report->started_at)
                            <div><small>Expertise démarrée le {{ date('d/m/Y à H:i', strtotime($report->started_at))}}&nbsp;|&nbsp;<span class="fw-semibold">{{ $report->technician?->username ?? '-' }}</span></small></div>
                            @endif
                            @if($report->closed_at)
                            <div><small>et terminée le {{ date('d/m/Y à H:i', strtotime($report->closed_at)) }}.</small></div>
                            @endif
                        </div>
                    </div>
                </div>
            ...
            <div id="po-{{ $report->id }}" popover="manual">
                ...
                {{ html()->form()->close() }}
                <div class="d-grid">
                    <button popovertarget="po-{{ $report->id }}" popovertargetaction="hide" class="btn btn-sm">Annuler</button>
                </div>
            </div>
        @endforeach
    @endif
